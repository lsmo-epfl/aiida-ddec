...
class DdecCp2kSpChargesWorkChain(WorkChain):
    """A workchain that computes DDEC charges after a single-point DFT calculation using CP2K"""

    @classmethod
    def define(cls, spec):
        """Define workflow specification."""
        super(DdecCp2kSpChargesWorkChain, cls).define(spec)
        ...
        spec.outline(cls.setup, cls.run_cp2k, cls.prepare_ddec, cls.run_ddec, cls.return_results)

        spec.expose_outputs(Cp2kMultistageWorkChain, include=('output_structure', 'remote_folder'))

        # specify the outputs of the workchain
        spec.output('output_structure', valid_type=CifData, required=True)
    ...
    def run_cp2k(self):
        ...
        inputs['protocol_tag'] = Str('sp')
        running = self.submit(Cp2kMultistageWorkChain, **inputs)
        self.report('pk: {} | Running Cp2kBaseWorkChain to compute the charge-density'.format(running.pk))
    ...
    def return_results(self):
        ...
        self.out('output_structure', self.ctx.ddec_calc.outputs.structure)
